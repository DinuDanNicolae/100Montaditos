...
public class WifeController : MonoBehaviour
{
    ...
    public GameObject startMenuButton; // Reference to the final menu button
    [SerializeField]
    public GameObject wifeGameObj;
    void Start()
    {
    ...
    if (other.gameObject.CompareTag("Player") && wifeRenderer.enabled)
    {
        Debug.Log("Collision with Player detected. Activating final screen.");
        Destroy(other.gameObject);
        Destroy(wifeGameObj);
        startMenuButton.SetActive(true);
        GameManager.IsGameOver = true; // Set the game over state
    }
